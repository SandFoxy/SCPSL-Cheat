...
#define NOTIFY_TOAST_FLAGS				ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing
	...
	NOTIFY_INLINE auto get_default_title() -> const char*
	{
		if (!strlen(this->title))
		{
			...
			case ImGuiToastType_Warning:
				return xorstr_("Warning");
			case ImGuiToastType_Error:
				return xorstr_("Error");
			case ImGuiToastType_Info:
				return xorstr_("Info");
			case ImGuiToastType_Hit:
				return xorstr_("Info");
			default:
				return NULL;
			}
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			char window_name[50]{};
			snprintf(window_name, sizeof(window_name), xorstr_("##TOAST%d"), i);
			
			//PushStyleColor(ImGuiCol_Text, text_color);
			if (current_toast->type == ImGuiToastType_Hit) {
				//opacity = 0.5f;
				SetNextWindowBgAlpha(opacity);


				float add = current_toast->get_fade_percent() * 40;

				if (height - add > vp_size.y / 2 - 161)
					continue;


				//GetBackgroundDrawList()->AddShadowRect(ImVec2(vp_size.x / 2 + (63 / 2), vp_size.y - NOTIFY_PADDING_Y - height + (current_toast->get_fade_percent() * 5)), ImVec2(vp_size.x / 2 + (63 / 2) + 63, vp_size.y - NOTIFY_PADDING_Y - height + (current_toast->get_fade_percent() * 5)+ 33), ImColor(255,255,255), 5, ImVec2(0,0), 0,5);
				SetNextWindowPos(ImVec2(vp_size.x / 2 + (63 / 2), vp_size.y - NOTIFY_PADDING_Y - height - add), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			}
			else {
				SetNextWindowBgAlpha(opacity);
			...
			PushStyleColor(ImGuiCol_Text,ImVec4(1,1,1,opacity));
			// Here we render the toast content
			{
				...
				if (!NOTIFY_NULL_OR_EMPTY(icon))
				{
					...
					TextColored(ImColor(255,255,255), icon);
				...
				if (!NOTIFY_NULL_OR_EMPTY(content))
				{
					if (was_title_rendered)
					{
						...
						PushStyleColor(ImGuiCol_Separator, ImVec4(1, 1, 1, opacity));
						Separator();
						PopStyleColor();
#endif
					}
